...
- **默认构建**：生成通用版本 `libreoffice-ofd-extension.oxt`
...
**生成**：`target/libreoffice-ofd-extension.oxt`
- 标识符：`org.libreoffice.ofd.extension`
- 显示名称：`LibreOffice OFD Extension`
...
```
- 生成：`target/loongoffice-ofd-extension.oxt`
- 标识符：`org.loongoffice.ofd.extension`

SmallOffice 版本：
```bash
mvn clean package -DskipTests -P branded -Dbrand=smalloffice
```
- 生成：`target/smalloffice-ofd-extension.oxt`
- 标识符：`org.smalloffice.ofd.extension`
...
# 3. 安装到 LoongOffice
/home/loongson/projects/loongoffice/build/instdir/program/unopkg add target/loongoffice-ofd-extension.oxt
```
...
**命名规范**：
- 所有版本都遵循 `<brand>-ofd-extension` 格式
- 默认版本（LibreOffice）：`libreoffice-ofd-extension`
- 品牌版本（LoongOffice）：`loongoffice-ofd-extension`
...
# 默认构建（LibreOffice 版本）
mvn clean package -DskipTests
# 输出: target/libreoffice-ofd-extension.oxt

# LoongOffice 品牌构建
mvn clean package -DskipTests -P loongoffice
# 输出: target/loongoffice-ofd-extension.oxt
```
...
#    - 上传 libreoffice-ofd-extension.oxt
#    - 添加发布说明
